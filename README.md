...
```text
                 / \
                /E2E\          Playwright — login, Start here practice, path UX
               /-----\
              / API   \        Playwright HTTP — auth, RBAC delete, XP, MCQ, perf smoke
             /---------\
            /   Unit    \      Vitest — XP/streak, RBAC, MCQ grading, Fastify inject
           /-------------\
```
...
| Soft learning path | `tests/e2e/learning-path.spec.ts` |
...
| ----- | -------- | ----- | ---- |
| Unit / inject | `tests/unit/` | 8 | Domain + Fastify inject (22 tests) |
| API | `tests/api/` | 5 | Live HTTP + perf smoke |
| E2E | `tests/e2e/` | 5 | Browser journeys + a11y |
| Cross-layer | `tests/cross-layer/` | 1 | Invite API → UI |
| PR gate | `@smoke` | 11 | lint + typecheck + unit + these |
...
| `specs/` | Spec Kit feature packages (`001`–`004`) |
